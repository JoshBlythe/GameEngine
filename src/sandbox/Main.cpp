...
struct movePlayer : public Component
{
	...
	void onTick()
	{
		...
		if (getCore()->getKeyboard()->getKey(SDL_SCANCODE_DOWN))
		{
			//update Camera Position - Move Backwards
			// std::cout << _currPos.x << _currPos.z << std::endl;
			//getEntity()->getTransform()->setPosition(glm::vec3(0, 0, _camMove));
		...
		if (getCore()->getKeyboard()->getKey(SDL_SCANCODE_RIGHT))
		{
			//update Camera Position - Move Backwards
			//getEntity()->getTransform()->setPosition(glm::vec3(_camMove, 0, 0));
			_currPos -= _camMove * _moveX;
		...
		//rotate Camara
		if (getCore()->getKeyboard()->getKey(SDL_SCANCODE_Z))
		{
			//update Camera Position - Move Backwards
			//getEntity()->getTransform()->setPosition(glm::vec3(_camMove, 0, 0));
			_currRot -= _camRot * _rotator;
		...
		if (getCore()->getKeyboard()->getKey(SDL_SCANCODE_X))
		{
			//update Camera Position - Move Backwards
			//getEntity()->getTransform()->setPosition(glm::vec3(_camMove, 0, 0));
			_currRot += _camRot * _rotator;
